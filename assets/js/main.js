...
function previsaoLocalizacao(latitude, longitude) {
    exibirPrevisao()
    const urlApi = `https://api.hgbrasil.com/weather?format=json-cors&key=abacbced&lat=${latitude}&lon=${longitude}&user_ip=remote`
...
function pesquisaPorNome(cidade, estado) {
    const urlApi = `https://api.hgbrasil.com/weather?format=json-cors&key=abacbced&city_name=${cidade},${estado}`
