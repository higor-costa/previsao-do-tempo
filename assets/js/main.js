...
function previsaoLocalizacao(latitude, longitude) {
    exibirPrevisao()
    const urlApi = `https://api.hgbrasil.com/weather?format=json-cors&key=f88d8233&lat=${latitude}&lon=${longitude}&user_ip=remote`
...
function pesquisaPorNome(cidade, estado) {
    const urlApi = `https://api.hgbrasil.com/weather?format=json-cors&key=f88d8233&city_name=${cidade},${estado}`
